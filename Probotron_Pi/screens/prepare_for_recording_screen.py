...
class PrepareForRecordingScreen( BaseScreen ):
    
        
    def setup( self ):
        super( PrepareForRecordingScreen, self ).select()
        
        self._duration = 3 # seconds by default.
    ...
    def reset( self ):
        super( PrepareForRecordingScreen, self ).reset()
    ...
    def _show_countdown (self):
        ...
        label1 = self._render_text( "Starting in", self._upper_font )
        l1w, l1h = label1.get_size()
        sw, sh = self._screen.get_size()
        # Each iteration of the countdown rendering changing text.
        myfont = pygame.font.SysFont("freesansbold.ttf", 250)
        label2 = myfont.render( str( time_remaining ) , True, (250, 227, 137) )
